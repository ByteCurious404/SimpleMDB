// Added by Copilot: actor list page JS for browsing and deleting actors.
// Added by Copilot: matches movie list behavior and pagination.
import {
  $,
  apiFetch,
  renderStatus,
  ...
  try {
    ...
    if (items.length === 0) {
      ...
      for (const a of items) {
        ...
        root.querySelector(".birth-year").textContent = String(a.birthYear ?? "—");
        root.querySelector(".btn-view").href =
          `/actors/view.html?id=${encodeURIComponent(a.id)}`;
        root.querySelector(".btn-edit").href =
          `/actors/edit.html?id=${encodeURIComponent(a.id)}`;
    ...
    listEl.addEventListener("click", async (ev) => {
      ...
      try {
        await apiFetch(`/actors/${encodeURIComponent(id)}`, {
          method: "DELETE",
        });
